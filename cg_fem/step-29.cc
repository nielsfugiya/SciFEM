                                     ...
                                     const unsigned int              component) const
  {
                                ...
                                const unsigned int component) const
   {
 	  if (dim == 2)
 	  {
 		  switch (component)
 		  {
 		  case 0: return cos(wave_number_2d.norm()*p.norm());
 		  case 1: return -sin(wave_number_2d.norm()*p.norm());
 		  default:
 			  abort ();
 		  }
 	  }
    ...
    for (; cell!=endc; ++cell)
      {
        ...
        for (unsigned int i=0; i<dofs_per_cell; ++i)
        {
           ...
           for (unsigned int j=0; j<dofs_per_cell; ++j)
           {
            	...
            	// Here we implement the right hand side of the weak form.
                if (component_i == 0)
                {
                  	 for (unsigned int q_point=0; q_point<n_q_points; ++q_point)
                  	 {
                  		 cell_rhs(i) += (fe_values.shape_value(i,q_point) *
                  				 	     rhs_values[q_point].real() *
                  				 	     (1-  refractive_index_values[q_point])*wave_number_2d * wave_number_2d*
                  				 	 	 fe_values.JxW(q_point));
                  	 }
                }
                else
